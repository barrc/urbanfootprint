...
class SwmmUpdaterTool(AnalysisTool, BuildingPerformance):
    ...
    def run_future_water_calculations(self, **kwargs):
        ...
        # self.printOut(features)
        # [<CanvasFeature65Rel: CanvasFeature65Rel object>, 
        # <CanvasFeature65Rel: CanvasFeature65Rel object>, 
        # <CanvasFeature65Rel: CanvasFeature65Rel object>, 
        # <Canva     21 sFeature65Rel: CanvasFeature65Rel object>, 
        # <CanvasFeature65Rel: CanvasFeature65Rel object>, 
        # <CanvasFeature65Rel: CanvasFeature65Rel object>, 
        # <CanvasFeatur     21 e65Rel: CanvasFeature65Rel object>, 
        # <CanvasFeature65Rel: CanvasFeature65Rel object>, 
        # <CanvasFeature65Rel: CanvasFeature65Rel object>, 
        # <CanvasFeature65Rel:     21  CanvasFeature65Rel object>, 
        # <CanvasFeature65Rel: CanvasFeature65Rel object>, 
        # <CanvasFeature65Rel: CanvasFeature65Rel object>, 
        # <CanvasFeature65Rel: Canvas     21 Feature65Rel object>, <CanvasFeature65Rel: CanvasFeature65Rel object>, <CanvasFeature65Rel: CanvasFeature65Rel object>, <CanvasFeature65Rel: CanvasFeature     21 65Rel object>, <CanvasFeature65Rel: CanvasFeature65Rel object>, <CanvasFeature65Rel: CanvasFeature65Rel object>, <CanvasFeature65Rel: CanvasFeature65Rel o     21 bject>, <CanvasFeature65Rel: CanvasFeature65Rel object>, '...(remaining elements truncated)...']
        ...
        # todo get the geometry and 
        # from {base_schema}.{base_table} a
        NLCD_type_list = []
        typeToAdd = {}
        ...
        for x in xrange(0,numRecords):
            ...
            # self.printOut(retVals[x]['center']) # {"type":"Point","coordinates":[-121.442411295164,38.438962169393]}
            # self.printOut('coordinates')
            # self.printOut('centerObj')
            # self.printOut(centerObj)
            # self.printOut(centerObj["coordinates"])
            # self.printOut(centerObj["coordinates"][0])
            # geom_dict[retVals[x]['id']] = {}
            # geom_dict[retVals[x]['id']]['geometry'] = retVals[x]['geom']
            # geom_dict[retVals[x]['id']]['type'] = "Feature"
            # geom_dict[retVals[x]['id']]['properties'] = {}
            # geom_dict[retVals[x]['id']]['properties']['cen_lat'] = centerObj["coordinates"][1] # "cen_lat": 38.3788, 
            # geom_dict[retVals[x]['id']]['properties']['cen_long'] = centerObj["coordinates"][0] # "cen_long": -121.3868 
            # geom_dict[retVals[x]['id']]['properties']['area'] = retVals[x]['area_sqm'] * 0.000247105  # needs to be converted to acres
            geom_dict = {}
            ...
            # {
            # 315904: {
            # 'geometry': u'{"type":"MultiPolygon","coordinates":[[[[-121.440276192969,38.440999590983],[-121.44027516758,38.4409851346927],[-121.440275772392,38.4409719700805],[-121.440285791066,38.4408734800647],[-121.44028690255,38.4408664106878],[-121.440288363598,38.4408607790373],[-121.440293111009,38.4408491489032],[-121.440691307039,38.4408742031376],[-121.440690894844,38.4410256828252],[-121.440276192969,38.440999590983]]]]}', 
            # 'type': 'Feature', 
            # 'properties': {'area': 0.2428271694958268, 'cen_long': -121.440486093668, 'nlcd_22': 1.0, 'cen_lat': 38.4409375142336}}, 

            # 315905: {'geometry': u'{"type":"MultiPolygon","coordinates":[[[[-121.442070131119,38.4409829409672],[-121.4421939296,38.4407235320023],[-1


            for t in NLCD_type_list:
            ...
            # {'id': 314860, 'st_asgeojson': u'{"type":"MultiPolygon","coordinates":[[[[-121.443025243479,38.4391236744712],[-121.442709679487,38.4390134062778],[-121.442724854674,38.4389866173649],[-121.442     42 738391714,38.43896146469],[-121.442751370036,38.4389361309411],[-121.4427710088,38.4388949644813],[-121.443092112298,38.4390076565006],[-121.443043170539,38.4390929843761],[-121.443025243479,38     42 .4391236744712]]]]}'}


        # self.printOut('geom_dict')
        # self.printOut(geom_dict)
        # {315904: {
        # 'geom': u'{"type":"MultiPolygon","coordinates":[[[[-121.440276192969,38.440999590983],[-121.44027516758,38.4409851346927],[-121.440275772392,38.440971970080                       5],[-121.440285791066,38.4408734800647],[-121.44028690255,38.4408664106878],[-121.440288363598,38.4408607790373],[-121.440293111009,38.4408491489032],[-121.4406913070                       39,38.4408742031376],[-121.440690894844,38.4410256828252],[-121.440276192969,38.440999590983]]]]}', 
        # 'id': 315904, 
        # 'center': u'{"type":"Point","coordinates":[-121.4404                       86093668,38.4409375142336]}'},
        ...
        for feature in annotated_features.iterator():
            ...
            # self.printOut('for feature in annotated_features')
            # self.printOut(feature)
            # self.printOut(feature.pop)
            # self.printOut(feature.hh)
            # self.printOut(feature.emp)

             # SELECT * FROM public.main_sacoglandusedefinition; land_use column

            # # from core_update_incremenet.py
            # self.printOut('feature.built_form_key')
            # self.printOut(feature.built_form_key) # bt__low_density_detached_residential
            # self.printOut('feature.built_form_id')
            # self.printOut(feature.built_form_id) # 233
            # self.printOut('feature.land_development_category')
            # self.printOut(feature.land_development_category) # standard
            # self.printOut('---------------------------------')
            # todo , need the geometry as geojson
            # self.printOut('adding impervious to properties')


            
            # for t in NLCD_type_list:
            #     if(feature.built_form_id in t['scag_codes']):
            #         geom_dict[feature.id]['properties'][t['nlcd_geojson_code']] = 1.0

            # geom_dict[feature.id]['properties']['impervious_code'] = feature.built_form_id
            # geom_dict[feature.id]['properties']['gid'] = 
            # geom_dict[feature.id]['properties']['gid'] = 
        ...
        # FeatureCollectionWrapper.crs.properties.name = "urn:ogc:def:crs:EPSG::4326"
        # { "type": "name", "properties": { "name": "urn:ogc:def:crs:EPSG::4019" } }
    ...
    def run_calculations(self, **kwargs):
        ...
        simulationID = False
        try:
            self.printOut("reading file")
            fileContentsStr = open('/srv/calthorpe/urbanfootprint/footprint/main/models/analysis_module/swmm_module/ElkGrove3.geojson', 'r').read()

            self.printOut("fileContentsStr")
            self.printOut(fileContentsStr)
            self.printOut("geo_json_str")
            self.printOut(geo_json_str)

            dumped = json.dumps(geo_json_str)
            self.printOut("geo_json_str dumped")
            self.printOut(dumped)
            
            # POST request to the submit endpoint with the form form_attribute filled with the geojson string
            # import requests
            # url = 'http://chis-dev.respec.com/rest/submit'
            # url = 'http://gisdev.respec.com:5007/rest/submit'
            # data = '{"form_attribute":{' + fileContentsStr + '}'
            # data = fileContentsStr
            # fileContentsStr = fileContentsStr.replace('\n', '')

            # fd = {"form_attribute": fileContentsStr }
            # data = { "form:" + fd }

            # s = Session()
            # payload = {'key1': 'value1', 'key2': 'value2'}

            # self.printOut("payload")
            # self.printOut(payload)
            ...
            # url = 'http://gisdev.respec.com:5007/rest/get_sim_list'
            # r = requests.post(url) # works without sending the geojson

            # r = requests.post(url, data={u"geojson_string": fileContentsStr})
            r = requests.post(url, data={u"geojson_string": dumped})
            ...
            # data = urllib.urlencode(fileContentsStr)
            # req = urllib2.Request(url, data)
            # response = urllib2.urlopen(req) 

            # self.printOut("resp")
            # self.printOut(resp)
            # self.printOut(resp.status_code)

            # data = '{"form_attribute":{"\'someVarName\'=\'somevarValue123\'"}'
...
# "form_attribute"
# ], 
# "sim_id": "sim-swmm-544277", 
# "success": true
# }
            # will return { "sim-id": "sim-swmm-3656125","success": true }  or { "success": false }
            self.printOut(r.status_code)
            self.printOut(r.reason)
            self.printOut('response')
            self.printOut(r)
            self.printOut(vars(r))
            try:
                self.printOut('r _content')
                self.printOut(r._content)
                responseObj = json.loads(r._content)
                # self.printOut('responseObj')
                # self.printOut(type(responseObj))
        ...
        self.printOut('now making get_sim_list call chis')
        try:
